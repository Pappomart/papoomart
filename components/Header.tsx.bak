...
export default function Header() {
  ...
  // Cierra el panel mobile al hacer click fuera o con ESC
  useEffect(() => {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 bg-white/90 backdrop-blur border-b border-gray-200">
      ...
      <div className="mx-auto max-w-6xl h-16 md:h-20 px-4 sm:px-6 md:px-8 flex items-center justify-between">
        {/* Logo sin fondo a la izquierda */}
        <Link href="/" aria-label="PapoomArt, ir al inicio" className="flex items-center gap-3">
          <Image
            src="/logo.png"           // usa tu archivo con transparencia en /public
            alt="PapoomArt"
            width={180}
            height={64}
            priority
        ...
        <nav className="hidden md:flex items-center gap-6 text-gray-800 text-base font-medium">
          ...
          {/* Tienda con submenú (hover y focus) */}
          <div className="relative group">
            ...
            >
              <Link
                ...
                role="menuitem"
                className="block rounded-md px-3 py-2 hover:bg-pink-50"
              >
      ...
      <div
        ref={panelRef}
        className={`md:hidden bg-white border-t border-gray-200 shadow-lg transition-transform duration-200 ${
          mobileOpen ? "translate-y-0" : "-translate-y-2 pointer-events-none opacity-0"
        }`}
      >
        <nav className="flex flex-col p-4 text-gray-900">
          ...
          <div className="border rounded-lg">
            ...
            >
              Tienda
              <FaChevronDown
                className={`text-xs transition-transform ${mobileSubOpen ? "rotate-180" : ""}`}
              />
            </button>
            ...
            <div id="submenu-tienda" className={`${mobileSubOpen ? "block" : "hidden"} px-2 pb-2`}>
              <Link
                href="/tienda/celebrar"
                onClick={() => setMobileOpen(false)}
                className="block rounded-md px-3 py-2 text-sm hover:bg-pink-50"
              >
                Etiquetas y tarjetas para celebrar
              </Link>
              <Link
