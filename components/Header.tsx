...
export default function Header({ offsetTop = 0 }: Props) {
  ...
  // Desktop (controlado por estado para evitar que se cierre al mover el mouse)
  const [deskTiendaOpen, setDeskTiendaOpen] = useState(false);
  const [deskCelebrarOpen, setDeskCelebrarOpen] = useState(false);
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-7xl h-20 px-4 sm:px-6 md:px-8 flex items-center justify-between">
        {/* Logo */}
        <Link href="/" aria-label="Ir al inicio — PapoomArt" className="flex items-center gap-3">
          <Image
            src="/logo.png"
            alt="PapoomArt"
            width={300}
        ...
        <nav className="hidden md:flex items-center gap-6 text-gray-800 text-base font-medium">
          ...
          {/* ====== TIENDA (dropdown controlado por estado) ====== */}
          <div
            className="relative"
            onMouseEnter={() => setDeskTiendaOpen(true)}
            onMouseLeave={() => {
              setDeskTiendaOpen(false);
              setDeskCelebrarOpen(false);
            }}
          >
            <button
              type="button"
              ...
              aria-haspopup="true"
              aria-expanded={deskTiendaOpen}
              onClick={() => setDeskTiendaOpen((v) => !v)}
            >
            ...
            {deskTiendaOpen && (
              <div
                ...
                "
              >
                {/* Bridge anti-gap: zona invisible encima del menú para no perder hover */}
                <div className="absolute -top-2 left-0 h-2 w-full" />

                {/* Celebrar con flyout */}
                <div
                  className="relative"
                  onMouseEnter={() => setDeskCelebrarOpen(true)}
                ...
                >
                  ...
                  {deskCelebrarOpen && (
                    <div
                      className="
                        absolute left-full top-0 ml-2 w-80 rounded-xl border border-gray-200 bg-white p-2 shadow-lg
                        z-50
                      "
                    >
                      ...
                      >
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">Etiquetas listas para tu fiesta</p>
                        </div>
                      </Link>
                      <Link
                      ...
                      >
                        ...
                        <div>
                          <p className="font-semibold text-gray-800 text-sm">Elige tus etiquetas</p>
                          <p className="text-xs text-gray-500">Tamaños, formas y cantidades</p>
                        </div>
                      </Link>
                    </div>
                  )}
          ...
          <nav className="flex flex-col p-4 text-gray-800">
            ...
            <div className="border-t border-gray-100 pt-3 mt-2">
              ...
              >
                Tienda
                <FaChevronDown
                  className={`text-xs transition-transform ${tiendaOpen ? "rotate-180" : ""}`}
                />
              </button>
